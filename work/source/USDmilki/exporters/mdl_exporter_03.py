...
class MdlExporter(Exporter):
    ...
    def pre_execute(self, targets, options):
        ...
        if pub_lastver != "None":
            self.ass_pub_path = re.sub(r"/v\d+/", "/"+pub_lastver+"/", self.ass_pub_path)
            self.ass_pub_path = re.sub(r"\_v\d+\.ass$", "_"+pub_lastver+".ass", self.ass_pub_path)
        
        all_path.append(self.ass_pub_path)
    ...
    def execute(self):
        ...
        # ass file publish
        ass_pub_dir_path = os.path.dirname(self.ass_pub_path)
        ...
        if self.PUB_WITH_MAT == False:
            ...
            ass_opt_list    = ['shape']
        neon.export_ass_v02(self.ass_pub_path, self.target, ass_opt_list)

        pub_targets.append(self.ass_pub_path)
