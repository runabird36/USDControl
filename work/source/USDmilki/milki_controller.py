...
from .utils import item_docker
reload (item_docker)
from .utils import item_supply
reload (item_supply)
from .toolkits import milki_sg_tk
reload (milki_sg_tk)
from .view import milki_widget
reload (milki_widget)
...
class MilkiController(QtCore.QObject):
    ...
    def __init__(self, app_name):
        ...
        self.milki_widget = milki_widget.MilkiWidget(self, app_name)
        self.milki_widget.setup()
        progress_dialog = self.milki_widget.get_progress_dialog()
        try:
            m_sg_tk = milki_sg_tk.MilkiSgTk()
        except:
            traceback.print_exc()
        self.supply = item_supply.ItemSupply(m_sg_tk, progress_dialog, self.app_name)
    ...
    def get_targets(self):
        ...
        if LUCY.get_category()== 'assets':
            self._IS_ASSET_ANI = True
        if (LUCY.get_pipe_step() in ['animation', 'matchmove', 'layout'] or self._IS_SHOT_MDL == True or self._IS_ASSET_ANI == False) and LUCY.get_pipe_step() not in ['simulation', "lighting"]:
    ...
    def select_all_export_targets(self, entity, targets):
        ...
        # Rig loc check
        if LUCY.is_sequence() == True and LUCY.get_pipe_step() in ["animation"]:
