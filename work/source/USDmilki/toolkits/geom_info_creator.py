...
def find_linked_texs(node, texs):
    ...
    if neon.get_inputs(node) is None:
        return
...
# def get_geom(sg):
#     meshes = cmds.listConnections(sg, source=True, destination=False, shapes=True, t='mesh')
#     paths = []
#     for mesh in meshes:
#         full_path = cmds.listRelatives(mesh, fullPath=True, allParents=True).pop()
