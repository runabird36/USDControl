...
class Expander(QtWidgets.QWidget):
    def __init__(self, parent=None, title='', tar_layout=None, animationDuration=150):
        """
        References:
        ...
        self.contents_layout    = tar_layout
        ...
        if self.contents_layout != None:
            self.setContentLayout(self.contents_layout)
        else:
            btn = QtWidgets.QPushButton("aaa")
            self.sub_layout = QtWidgets.QVBoxLayout()
            self.sub_layout.addWidget(btn)
            self.setContentLayout(self.sub_layout)
